...
export default async function FeedPostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: post } = await supabase
    .from('posts')
    .select('*, profiles(username, full_name, avatar_url, verified, github_connected, role, location)')
    .eq('id', id)
    .maybeSingle()
      ...
      <div style={{ minHeight: '100vh', background: '#fbfbfd', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
        <div style={{ maxWidth: 680, margin: '0 auto', padding: '4rem 1.5rem 5rem' }}>
          ...
          <div style={{ background: 'white', border: '1px solid #e0e0e5', borderRadius: 16, padding: '2rem' }}>
            ...
            {/* Reactions — static display on permalink */}
                ...
                {(['🔥','🚀','🛠️','👍'] as string[]).map(emoji => {
                  ...
                  return (
                    <div key={emoji} style={{ background: '#f5f5f7', border: '1px solid #e0e0e5', borderRadius: 980, padding: '0.3rem 0.65rem', fontSize: 13, display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                      {emoji}
                      <span style={{ fontSize: 12, color: '#6e6e73', fontWeight: 500 }}>{reactions[emoji]}</span>
                    </div>
                  )
                })}
              </div>
          ...
          <div style={{ marginTop: '1.5rem', background: 'white', border: '1px solid #e0e0e5', borderRadius: 14, padding: '1.25rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
            <div>
              <p style={{ fontSize: 14, fontWeight: 600, color: '#1d1d1f', marginBottom: '0.15rem' }}>
                More builds from {profile?.full_name?.split(' ')[0]}
              </p>
