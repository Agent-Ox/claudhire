...
function EmployerMessagesInner() {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, (payload) => {
        ...
        if (selectedRef.current && newMsg.conversation_id === selectedRef.current.id) {
          ...
          fetch(`/api/messages/${selectedRef.current.id}`).catch(() => {})
        }
        setConversations(prev => {
          const updated = prev.map(c => c.id === newMsg.conversation_id
          ...
          {conversations.map(conv => {
            ...
            return (
              <div key={conv.id} onClick={() => openConversation(conv)}
                style={{ padding: '1rem 1.25rem', cursor: 'pointer', borderBottom: '0.5px solid #f0f0f5', background: selected?.id === conv.id ? '#f0f5ff' : 'white' }}>
                <div style={{ display: 'flex', gap: '0.65rem', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <p style={{ fontSize: 14, fontWeight: 600, color: '#1d1d1f', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.full_name || 'Builder'}</p>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', flexShrink: 0 }}>
                        {conv.last_message && <p style={{ fontSize: 12, color: '#aeaeb2' }}>{timeAgo(conv.last_message.created_at)}</p>}
                        {conv.unread_count > 0 && (
                          <span style={{ fontSize: 11, fontWeight: 700, background: '#0071e3', color: 'white', borderRadius: '50%', width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{conv.unread_count}</span>
                        )}
                      </div>
                    </div>
  ...
  const Thread = () => {
    const b = selected ? builder(selected) : {}
    const initials = b.full_name?.split(' ').map((n: string) => n[0]).join('').slice(0, 2).toUpperCase() || '?'
    return (
        ...
        <div style={{ padding: '1rem 1.25rem', borderBottom: '0.5px solid #e0e0e5', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <button onClick={() => setShowThread(false)}
              style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 20, color: '#6e6e73', padding: 0, display: 'flex', alignItems: 'center' }}>
              ←
            </button>
          ...
          {b.username && (
            <a href={`/u/${b.username}`} target="_blank" style={{ fontSize: 12, padding: '0.3rem 0.75rem', background: '#f5f5f7', color: '#1d1d1f', borderRadius: 980, textDecoration: 'none', fontWeight: 500, whiteSpace: 'nowrap' }}>
              View →
            </a>
          )}
        </div>
          ...
          {messages.map(msg => {
            ...
            return (
              <div key={msg.id} style={{ display: 'flex', justifyContent: isMe ? 'flex-end' : 'flex-start' }}>
                <div style={{ maxWidth: '80%', background: isMe ? '#0071e3' : '#f5f5f7', color: isMe ? 'white' : '#1d1d1f', borderRadius: isMe ? '16px 16px 4px 16px' : '16px 16px 16px 4px', padding: '0.65rem 1rem', fontSize: 14, lineHeight: 1.5 }}>
                  <p>{msg.content}</p>
      ...
      <div className="msg-page-wrap" style={{ maxWidth: 900, margin: '0 auto', padding: '4rem 1.5rem 2rem' }}>
        ...
        <style>{`
          @media (max-width: 640px) {
            ...
            .msg-mobile-list { display: ${showThread ? 'none' : 'block'} !important; height: calc(100dvh - 220px); }
            .msg-mobile-thread { display: ${showThread ? 'block' : 'none'} !important; height: calc(100dvh - 220px); }
          }
          @media (max-width: 640px) {
          ...
          @media (min-width: 641px) {
            ...
            .msg-mobile-thread { display: none !important; }
            .msg-desktop { display: grid !important; }
          }
        `}</style>

        {/* Desktop */}
        <div className="msg-desktop" style={{ gridTemplateColumns: '300px 1fr', gap: '1rem', height: 'calc(100vh - 200px)', minHeight: 500, display: 'grid' }}>
          <ConvList />
        ...
        {/* Mobile */}
        <div className="msg-mobile-list"><ConvList /></div>
        <div className="msg-mobile-thread">{selected && <Thread />}</div>
      </div>
    </div>
  )
}
