# The Atlas of the Agentic Economy

**Version 0.5 — Practitioner-defined**
...
Published May 15, 2026 · Updated May 17, 2026 (v0.5) · ~67 min read
...
Between v0.4 and v0.5, something happened that is worth saying at the front, because it bears on whether this Atlas is describing real structure or just one practitioner's framing. In May 2026, Microsoft and GitHub launched a role-based certification for the work this Atlas calls the supervisory layer — and they did it from their own job-task analysis, with no knowledge of or interest in this document. When an institution that large draws the same boundary you drew, working from the opposite direction, that is not proof you are right. But it is evidence that the thing being mapped is real and not invented. v0.5 records that, and is otherwise a consolidation: the version is stamped, the validation is integrated where it belongs, and the crosswalk framing is sharpened. The larger structural additions remain planned for v0.6.
...
- **The frontier labs are the leading indicator.** Most of the structural specialization in this Atlas was first observed in actual hiring at Anthropic, OpenAI, Palantir, Databricks, Cohere, and the leading AI-native code-tooling companies. What's at the labs now is at customers in 12-24 months.
- **Crosswalks are provided where confident, flagged where not.** v0.4 adds ISCO-08, SOC 2018, and O*NET codes per role where a confident mapping exists. Where no clean mapping exists, the gap is named explicitly — that gap is one of the reasons this Atlas needs to exist. v0.5 adds a second observation to the same register: where an industry certification has formalized a role before any occupational code exists, that ordering is itself the finding.
- **This is v0.5.** Pull requests welcome.
...
**Certification-before-classification (NEW in v0.5).** A second signal joins the crosswalk register. For some roles, an industry certification body has now formalized the role — defined its competencies, weighted them, built an exam — while every occupational taxonomy still shows a gap. When a credential exists before a code exists, the ordering is not an accident. It is the labor market's own institutions confirming, ahead of the statistical agencies, that the role is real. Where this has happened, the role's crosswalk note records it. The first instance, in May 2026, is the Microsoft/GitHub GH-600 certification; see Mechanism M3.
...
**External signal (NEW in v0.5).** The supervise-the-agent-fleet boundary that defines where this role survives automation was independently formalized in May 2026 by the Microsoft/GitHub GH-600 role-based certification (operating, supervising, and governing AI agents across the SDLC). See Mechanism M3, *External validation*.

**Crosswalks.** ISCO-08: 2522 (Systems administrators) — partial. SOC 2018: 15-1241 (Computer Network Architects) — gap. O*NET: 15-1244.00 (Network and Computer Systems Administrators) — partial. `crosswalk: partial — AI-specific SRE work is structurally distinct from traditional SRE but taxonomies have not absorbed the distinction. Note (v0.5): a role-based industry certification covering this supervisory work now exists (Microsoft/GitHub GH-600, 2026) before any occupational code does — the certification-before-classification pattern`.
...
**External signal (NEW in v0.5).** GH-600's highest-weighted exam domain is MCP server competency — the exact failure surface (MCP server failures, tool-call errors, multi-agent orchestration) this role is defined by. An industry certification weighting MCP most heavily is independent field evidence that this is where the production agent-reliability work concentrates. See Mechanism M3, *External validation*.

**Crosswalks.** `crosswalk: gap`. No agent-specific reliability code exists in any official taxonomy. Note (v0.5): the Microsoft/GitHub GH-600 certification (2026) now formalizes much of this competency surface, MCP-weighted, before any occupational code exists — certification-before-classification.
...
**External signal (NEW in v0.5).** "Operating, supervising, and governing AI agents" is the verbatim shape of the Microsoft/GitHub GH-600 certification launched May 2026 — and GH-600's governance domain (autonomy levels, human-in-the-loop boundaries, auditability) is precisely the C4 boundary. An institution drawing the governance line independently is corroboration that the steward role is real structure, not framing. See Mechanism M3, *External validation*.

**Crosswalks.** `crosswalk: gap`. No agent-stewardship code exists in any taxonomy. Note (v0.5): the role's governance core is now formalized in the Microsoft/GitHub GH-600 certification (2026) — a credential before a code, the certification-before-classification pattern.
...
**External signal (NEW in v0.5).** GH-600's multi-agent orchestration domain — conflict detection on a shared codebase, recovery patterns, tool/permission/environment configuration for agents — is the architectural surface this role owns. The certification treating it as a distinct, examinable competency is independent confirmation that multi-agent architecture is a recognized job, not a sub-task of generic software architecture. See Mechanism M3, *External validation*.

**Crosswalks.** ISCO-08: 2511 (Systems analysts) — partial. SOC 2018: 15-1299.08 (Computer Systems Engineers/Architects) — partial. `crosswalk: partial. Note (v0.5): the multi-agent-architecture competency is now formalized in the Microsoft/GitHub GH-600 certification (2026) ahead of any occupational code — certification-before-classification`.
...
**External validation (NEW in v0.5).**

This section makes a claim that is easy to write and hard to prove: that the supervisory layer of the agentic economy is a real, mis-classified role — not a framing artifact, not a way of making generic engineering sound new. Between v0.4 and v0.5, that claim received the strongest external corroboration it has had to date, and it is worth setting down precisely, because it arrived from the opposite direction to this Atlas.

In May 2026, Microsoft and GitHub launched a role-based certification: GitHub Certified: Agentic AI Developer, Exam GH-600, in beta, with general availability scheduled for July 2026. It certifies expertise in operating, integrating, supervising, and governing AI agents inside production software-development workflows, with GitHub as the system of record and control plane. Two features of it are the Atlas's own argument restated by someone who was not making the Atlas's argument.

First, it is explicitly role-based, not tool-based. GitHub's own description of the shift the certification exists to validate is that the required skills are moving "from writing code to designing, supervising, and improving systems that write and operate on code." That is, almost to the word, the boundary this Atlas drew in v0.4 around the B-cluster, around C4, and around D2 — the line between the work that collapses to an agent fleet and the supervisory, governing, architecting work that does not. A certification body does not write that sentence to be provocative. It writes it because its job-task analysis produced it.

Second, the highest-weighted domain in the GH-600 exam is MCP — Model Context Protocol — server competency. This Atlas has argued, in this section, that a named role only becomes routable infrastructure once its identity is referenced by other systems, and that the protocol layer is what makes that referencing possible. We did not need a certification to weight MCP highest to believe that; but a certification body weighting it highest, from observed job-task frequency rather than from a thesis, is field evidence for the infrastructure claim, gathered independently of the Atlas and pointing the same way.

The significance is not the credential. Credentials come and go, and this Atlas is not in the business of endorsing one. The significance is the shape of what happened. An institution with no stake in this document, working from its own analysis of what people in this work actually do, had to draw a single role boundary *across* five existing job titles — software developer, platform engineer, DevOps engineer, security engineer, technical product manager — because the work does not sit inside any one of them. Drawing a new boundary across old titles, because the work outran the titles, is the entire thesis of this section. It has now been done, in public, by someone who was not trying to confirm it. That is the strongest kind of validation a map can get: not a reviewer agreeing with the map, but a second cartographer, working blind, drawing the same coastline.

One consequence for the rest of this Atlas, recorded in the crosswalk register and worth stating here in plain terms: for the roles GH-600 most directly covers, the labor market now has a *certification* before it has an *occupational code*. ISCO-08, SOC 2018, and O*NET still show gaps for this work. A credential exists; a classification does not. That ordering — the industry certifying a role the statistical agencies have not yet named — is not a contradiction of the crosswalk gaps reported in v0.4. It is the same finding, one step further along: the labor market's own institutions are now formalizing these roles ahead of the official taxonomies, which is exactly what you would expect if the central claim of this Atlas is true.
...
But — and this is critical — the operator pattern is also visible at scaleup level. **An AI-native code-tooling scaleup at ~$2B ARR with ~50 employees** is the operator pattern at scaleup scale. **Midjourney at $500M revenue with ~107 employees** is the operator pattern at scaleup scale. The pattern isn't just solo. The pattern is *small humans + many agents*, at every size of company.
...
A note worth flagging: at AI-native companies under 50 people (Midjourney, early-stage everywhere), the Atlas roles aren't separate hires. They're aspects of one polymath role. The founding engineer ships features, embeds with strategic customers (FDE work), tunes prompts (D3), runs evals (D5), manages production reliability (B-cluster). The AI-native-scaleup culture pattern: "ship-fast, eng-driven, product-impact, flat hierarchy, many-hats." The Atlas describes the specializations but recognizes that early-stage AI-native companies don't separate them.
...
## v0.6 planned additions (next major Atlas iteration)

*v0.5 consolidated: it stamped the version, integrated the GH-600 external validation into Mechanism M3 and the roles it most directly corroborates (B1, B2, C4, D2), and sharpened the crosswalk register with the certification-before-classification observation. The structural additions below were planned for v0.5 and are now carried to v0.6 — they require new field observation, which is the part of this Atlas that has to be witnessed, not compiled.*
...
- **AI Policy / Government Affairs roles** distinct from Part III compliance. The role exists at frontier labs (Government Affairs, Public Policy) and increasingly at large customers; it is policy-shaping, not policy-compliance.
- **AI-Native Product Designers** as distinct from D4 (Human-AI Handoff Designer). The UX work for agentic interfaces is a recognizable specialization at AI-native scaleups.
- **Embedded ML Engineers** — edge AI, on-device inference, model compression specialists. Distinct from Part VI vertical specialists; horizontal across verticals.
- **AI Hardware Engineers** — accelerator design, GPU systems engineering. NVIDIA, Cerebras, Groq, custom accelerator companies.
- **Expanded compensation refresh** across all roles with current 2026 Levels.fyi data per role per geography per company.
- **Frontier-Lab-Specific Org Maps.** Anthropic's 9+ safety specialisms documented at sub-role level. OpenAI's vertical FDE org documented per vertical. Palantir's Echo/FDE pod structure documented at delivery level. The leading-indicator detail that customer-facing taxonomies need 12-24 months from now.
- **Sovereign AI labor patterns.** Mistral, Aleph Alpha, sovereign EU/UK/Asian labs and their distinct labor-surface characteristics relative to US frontier labs.
- **Continued crosswalk refinement.** Identify and report the gap roles to ISCO and SOC maintainers, with the goal of seeing official codes added for the most structurally important new occupations. v0.5's certification-before-classification finding (GH-600) is the first concrete data point to bring to those maintainers: an industry credential now exists for work the statistical agencies have not classified.
- **Atlas API.** Each role dereferenceable as JSON-LD at shipstacked.com/atlas/roles/{ID}. Other systems can cite Atlas role IDs in their own taxonomies. The Atlas becomes infrastructure.
